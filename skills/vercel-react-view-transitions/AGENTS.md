...
Page stays mounted on dynamic segment change — enter/exit never fire. Use `key` + a **static** `name` + `share`:
...
The `name` must be static (`"collection-content"`), not per-slug. A crossfade only forms when the old and new VTs share the *same* `name`; a per-slug name (`collection-${slug}`) gives each segment a different name, so no pair forms and nothing animates. `enter="auto"` covers the first render where no old pair exists.
