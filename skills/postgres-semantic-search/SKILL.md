...
- **Prefix tsquery** for languages with rich inflection (no full morphology engine needed): build the tsquery by hand with `:*` on each token, so `kartta:*` matches `kartta`, `karttaa`, `karttoja`. `websearch_to_tsquery` cannot emit `:*`. Use the hardened `prefix_tsquery` in [fuzzy-search.md](references/fuzzy-search.md#prefix-matching-for-agglutinative-languages) rather than hand-rolling one — it handles three things that each silently return zero rows: tsquery metacharacters in raw input (`&`, `|`, `!`, `(`, `:` raise a syntax error), AND-joining terms (measured: zero rows for every realistic multi-word query in an inflected language — OR-join instead), and hyphenated tokens, which `to_tsquery` turns into a `<->` phrase that matches nothing.
